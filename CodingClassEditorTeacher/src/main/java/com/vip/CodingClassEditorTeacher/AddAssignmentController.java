...
public class AddAssignmentController implements Initializable{
	...
	public void addPracticalButtonAction() {
		if (InputValidation.validateAddPractical(practicalName, practicalnameerror, time, timeerror, practicalDate, dateerror)) {
			...
			practical.setAssignmentName(practicalName.getText().trim());
			LocalDate ld = practicalDate.getValue();
			Calendar c =  Calendar.getInstance();
			c.set(ld.getYear(), ld.getMonthValue()-1, ld.getDayOfMonth());
			Date date = c.getTime();
			practical.setAssignmentDate(date);
	...
	private void addPractical(Assignment practical) {
		// TODO Auto-generated method stub
		Call<Object> call = teacherService.addAssignment(practical);
			...
			public void onResponse(Call<Object> call, Response<Object> response) {
				...
				if (response.isSuccessful()) {
					Platform.runLater(new Runnable() {
						...
						@Override
						public void run() {
							if (response.body().toString().contains("assignment added sucessfully")) {
								...
								if (response.body().toString().contains("assignment already exists")) {
									addPracticalError.setText("assignment already exists");
